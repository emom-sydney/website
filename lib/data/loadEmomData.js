...
function buildNormalizedData({
  ...
  const normalizedEvents = events.map((event) => ({
    ...event,
    Date: event.Date || formatLegacyEventDate(event.event_date),
    TypeID: event.TypeID ?? event.type_id,
    EventName: event.EventName ?? event.event_name,
    GalleryURL: event.GalleryURL ?? event.gallery_url ?? "",
  }));
...
function loadFromCsv() {
  return buildNormalizedData({
    artists: parseCsvTable("artists.csv"),
    artistimages: parseCsvTable("artistimages.csv"),
    artistsocialprofiles: parseCsvTable("artistsocialprofiles.csv"),
    events: parseCsvTable("events.csv"),
    eventtypes: parseCsvTable("eventtypes.csv"),
    performances: parseCsvTable("performances.csv"),
    socialplatforms: parseCsvTable("socialplatforms.csv"),
  });
}
