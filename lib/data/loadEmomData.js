...
function getPublicProfileWhereClause(alias = "prof") {
  return `
      ${alias}.is_profile_approved = true
      AND ${alias}.is_profile_index_visible = true
      AND (${alias}.profile_visible_from IS NULL OR ${alias}.profile_visible_from <= CURRENT_DATE)
      AND ${alias}.profile_expires_on >= CURRENT_DATE
  `;
}
    ...
    if (!page) {
      page = {
        profile: {
          ...
          tribuoTag: row.tribuo_tag,
          isProfileApproved: row.is_profile_approved,
          isProfileIndexVisible: row.is_profile_index_visible,
        },
        image: row.image_id ? {
          ID: row.image_id,
...
function getRolePagesQuery(role, { includePerformances = false, publicOnly = false } = {}) {
...
`
    : `
        NULL::integer AS performance_id,
        NULL::integer AS event_id,
        NULL::integer AS type_id,
        NULL::text AS event_name,
        NULL::text AS gallery_url,
...
`
  ...
  const publicWhereClause = publicOnly
    ? `
      WHERE (
        ${getPublicProfileWhereClause("prof")}
        OR EXISTS (SELECT 1 FROM event_performer_selections s WHERE s.profile_id = prof.id AND s.status = 'selected')
      )
`
    : "";
  ...
  return `
      SELECT
        ...
        prof.is_name_public,
        prof.tribuo_tag,
        prof.is_profile_approved,
        prof.is_profile_index_visible,
        ai.id AS image_id,
        ai.image_url,
...
function buildNormalizedData({
  ...
  const normalizedEvents = events.map((event) => ({
    ...
    startsAt: event.starts_at ?? event.startsAt ?? null,
    endsAt: event.ends_at ?? event.endsAt ?? null,
    timezone: event.timezone ?? "Australia/Sydney",
    locationID: event.location_id ?? event.locationID ?? null,
  ...
  const artistPages = artistPagesInput.map((artistPage) => {
    ...
    return {
      ...
      hasPastPerformance,
      isPubliclyVisible: Boolean(artistPage.profile.isProfileIndexVisible),
    };
  });
...
function getPostgresConfig() {
  // Loads the PG config explicitly
  dotenv.config({ path: '.pgenv' });
  ...
  try {
    const artistsResult = await client.query(`
      ...
      JOIN profile_roles pr
       ...
       AND pr.role = 'artist'
      WHERE ${getPublicProfileWhereClause("p")}
      ORDER BY id
    `);
    ...
    const eventsResult = await client.query(`
      SELECT
        ...
        NULLIF(BTRIM(youtube_embed_url), '') AS "YouTubeEmbedURL",
        e.event_date,
        e.starts_at,
        e.ends_at,
        e.timezone,
    ...
    const artistPagesResult = await client.query(
      getRolePagesQuery("artist", { includePerformances: true, publicOnly: true })
    );
    const volunteerPagesResult = await client.query(getRolePagesQuery("volunteer"));
    const tribuoSettingResult = await client.query(`
      SELECT value_json
      FROM app_settings
      WHERE key = 'tribuo_base_url'
    `);
    const tribuoBaseUrl = tribuoSettingResult.rows[0]?.value_json || null;
    ...
    const galleryEventsResult = await client.query(`
      SELECT
        ...
        e.event_name,
        NULLIF(BTRIM(e.gallery_url), '') AS gallery_url,
        NULLIF(BTRIM(e.youtube_embed_url), '') AS youtube_embed_url,
        e.event_date,
        p.id AS profile_id,
        COALESCE(p.display_name, perf.performer_display_name) AS display_name,
        perf.sort_order
      FROM events e
      LEFT JOIN performances perf
      ...
      ORDER BY e.event_date, e.id, perf.sort_order NULLS LAST, perf.id
    `);
    ...
    for (const row of galleryEventsResult.rows) {
      ...
      if (row.display_name && !galleryEvent.artists.some((artist) => artist.ID === row.profile_id && artist.stageName === row.display_name)) {
